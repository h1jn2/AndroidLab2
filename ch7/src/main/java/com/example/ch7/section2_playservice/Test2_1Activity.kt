package com.example.ch7.section2_playservice
...
class Test2_1Activity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.locationButton.setOnClickListener {
            ...
            // 현 시점 디바이스에 이용 가능 제공자가 없을 때 콜백
            val failedCallback = object : GoogleApiClient.OnConnectionFailedListener {
                override fun onConnectionFailed(p0: ConnectionResult) {
                    Toast.makeText(this@Test2_1Activity, "가용할 제공자 없음", Toast.LENGTH_SHORT).show()
                }
            }

            // GoogleApiClient 초기화 하면서 준비한 콜백 등록
            val apiClient: GoogleApiClient = GoogleApiClient.Builder(this)
                .addApi(LocationServices.API)   // play-service 의 다양한 api 중 어느 api 사용하는 지
                .addConnectionCallbacks(connectionCallback)
                .addOnConnectionFailedListener(failedCallback)
                .build()

            // 결과는 각종 등록된 콜백이 호출 되면서 위치 정보 제공자 결정
            apiClient.connect()
        }
    }
}